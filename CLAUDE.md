...
- **Done** — one line per shipped unit. Full detail (decisions, migration numbers, verification) lives in `progress.md`; keep it that way: **a finished slice adds ONE line here, the narrative goes in `progress.md`.**
  ...
  - Next.js 16 upgrade (branch `upgrade-next-16`, 2026-08-26): next 16.3.3 + react 19.2.8, `middleware.ts`→`proxy.ts`, Turbopack dev+build (`transpilePackages` workaround dropped), React Compiler on (in vitest too; 16 pre-existing hooks violations demoted to warn → `t-0c7ire`), typegen `PageProps`, `next typegen` before typecheck. Follow-ups closed 2026-08-26 on develop: react-hooks cleanup 16/16 + rules back to `error` (`28d01ea`), `<Activity/>` Board⇄List state retention (`0ed0055`), TS7 evaluated→**deferred** (typescript-eslint blocker; adoption = `t-0c7kct`). Cache Components stays deferred per `d-0c7hxl`.
  ...
  - Extension answer surfacing (branch `slice-extension-answers`, merged to develop 2026-08-28, `c22c4a7`): saved answers inside the popup — context-preselected tabs (`d-0c9ove`), form-wide question scan with a switcher, local bigram ranking (`d-0c9ovf`), insert through the **native value setter** (verified against live Greenhouse/Ashby — naive assignment fires React's onChange zero times). Two thin `/api/extension/answers` routes delegate to `answersService`; **permissions unchanged**. Rich-text fields still insert as `textContent` (`t-0cb5xk`).
  - Global search (branch `slice-global-search`, 2026-08-28): `GET /api/search` — one user-scoped `UNION ALL` over five tables, Postgres FTS blended with `pg_trgm` at query time (`d-0c5wyy`: no stored `tsvector`, no GIN, **no sync code**), ranked in **two bands** so exact beats fuzzy; migration `0014` is just `CREATE EXTENSION pg_trgm`. `ts_headline` emits `\x02`/`\x03`, never HTML — `snapshot_markdown` is scraped, so `<b>` would be stored XSS. Morphing ⌘K palette built **over `DialogPrimitive`** (`d-0cbc74`), origin *and* destination measured (`left:50%` is viewport-relative and the rail offsets the column). `t-0021` dropped out of scope: `?resume=` already worked. Follow-ups on develop: constant radius + a close that reverses the morph back into the trigger (`d0cc609`); **partial matching** (`t-0cbm48`, merged `8a637a6`, 2026-08-29) — a prefix arm OR'd into the tsquery so `reac` finds *React* (bodies included, highlighted free by `ts_headline`) plus one `ILIKE` band over the short identifying columns so `gine` finds *Engineer*, **three** bands now (FTS `2.0+ts_rank` · substring `1.0+similarity` · trigram floored at 0.3) and the offsets are load-bearing, not cosmetic. No migration, no new dependency, no frontend change. Knowingly accepts a measured multi-word `ts_rank` regression — OR-ing the arms moves the query root to `OP_OR`, so proximity stops boosting adjacent matches (`d-0ccyjm`; the undo, with three ways out, is `t-0cczse`).
  - Mobile wave 1 (merged to develop 2026-08-29): C0 Expo foundation (SDK 57, NativeWind, four-tab shell per `d-0cd3wr`) · native token auth transport · push delivery (`device_tokens`, migration `0015`). Uncovered a live auth defect: refresh tokens were **bcrypt-hashed and bcrypt truncates at 72 bytes**, so two refresh JWTs for one user compared equal and reuse detection never fired — and that masked `users.refresh_token_hash` being a single column, i.e. multi-device was impossible. Replaced by `user_sessions` (migration `0016`) + SHA-256; two review rounds then found four more HIGH issues, incl. **access and refresh tokens being interchangeable** (a refresh token was a valid `Bearer` credential, so revocation was decorative). Rotation is now one atomic UPDATE with a 15s one-token-deep grace window whose grace arm returns an access token only (`d-0cdcga`). Mobile palette needed the web's two-block token shape — `light-dark()` is unsupported by `react-native-css` and every gate passed while nothing rendered; the tab bar corner was mirrored and then invisible until the border came off and the bar went dark (`d-0cd3wr` amendments 1–2).
- **Next:** the **mobile app** — milestone `m-0cc02t`, **scope settled and every architectural decision recorded**; read `docs/superpowers/specs/2026-08-28-mobile-app-expo-scope.md` §8 first, it is written to be read cold. **C0 is done and merged** — the next wave is C1 auth (`t-0ccxkl`) and C2 primitives (`t-0ccxkm`), which both write `mobile/src` and so need explicit file ownership per lane. Two wave-1 tasks are held back on purpose: `t-0cdegw` (dark mode) is user-deferred, and `t-0cdfyq` (tab bar polish) waits on the user naming the adjustments. C1 inherits two contracts from `d-0cdcga`: native logout carries **no body** — `sid` rides on the access token, and the fail-closed arm that kills every session fires on a *missing `sid`*, not a missing body (`d-0cdcga` amendment 2026-09-01, `t-0cgtgo`) — and when a refresh response omits `refreshToken` (the grace arm) the client **keeps** its stored one. Expo/React Native, not Flutter (`d-0cc01s`); `mobile/` standalone, not a workspace (`d-0cc2vk`); Android first (`d-0cc2w5`). After that **Google OAuth** [`t-0020`] and **email reminders** [`t-0001`, the most user-valuable deferred item]. **The backlog now lives in `.blink/tasks/`, one file per item** — `docs/deferred-tasks.md` and `docs/polish-and-tech-debt.md` are just pointers. (User pushes — local master ahead of `origin/master`.)
...
- **Design:** minimalist-ui — warm-stone base, **flat muted-indigo** accent, **Geist** (sans) + **Geist Mono** (numerics signature) + **Newsreader** (editorial headings), faint hairline borders, near-zero diffuse shadows, dark-mode first-class. Use the `minimalist-ui` skill.
