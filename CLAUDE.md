...
- **Next:** **partial/substring search** [`t-0cbm48` — FTS matches whole lexemes, so `reac` finds nothing; needs a substring band beside FTS, on short fields only], then **Google OAuth** [`t-0020`] and **email reminders** [`t-0001`, the most user-valuable deferred item]. **The backlog now lives in `.blink/tasks/`, one file per item** — `docs/deferred-tasks.md` and `docs/polish-and-tech-debt.md` are just pointers. (User pushes — local master ahead of `origin/master`.)
