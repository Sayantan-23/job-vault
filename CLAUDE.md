...
- **Next:** **global search** [`t-0c5wyz`, Postgres FTS per decision `d-0c5wyy`; it also builds `/app/resumes/[id]` and a `?persona=` slideover], then **Google OAuth** [`t-0020`] and **email reminders** [`t-0001`, the most user-valuable deferred item]. **The backlog now lives in `.blink/tasks/`, one file per item** — `docs/deferred-tasks.md` and `docs/polish-and-tech-debt.md` are just pointers. (User pushes — local master ahead of `origin/master`.)
