...
function row(over: Partial<CoverLetterRow> = {}): CoverLetterRow {
  return { id: 'cl1', userId: 'u1', jobId: 'j1', adhocJob: null, personaId: 'p1', title: 'Acme', instructions: null, bodyMarkdown: 'Dear…', createdAt: new Date(), updatedAt: new Date(), ...over }
}
...
describe('cover-letters routes', () => {
  ...
  it('generates from a pasted JD (201, adhoc — no job lookup)', async () => {
    personas.findById.mockResolvedValue(persona)
    ai.generateText.mockResolvedValue('Dear hiring manager')
    repo.create.mockResolvedValue(row({ jobId: null, adhocJob: { title: 'Staff Eng', company: 'Acme', description: 'JD' } }))
    const res = await request(app)
      .post('/api/cover-letters')
      .set('Cookie', [cookie])
      .send({ personaId: UUID, job: { title: 'Staff Eng', company: 'Acme', description: 'JD' } })
    expect(res.status).toBe(201)
    expect(res.body.data.jobId).toBeNull()
    expect(res.body.data.adhocJob).toEqual({ title: 'Staff Eng', company: 'Acme', description: 'JD' })
    expect(jobs.findById).not.toHaveBeenCalled()
  })
  it('400 when both jobId and job are sent', async () => {
    const res = await request(app)
      .post('/api/cover-letters')
      .set('Cookie', [cookie])
      .send({ personaId: UUID, jobId: UUID, job: { title: 'T', company: 'C' } })
    expect(res.status).toBe(400)
  })
  it('400 when neither jobId nor job is sent', async () => {
    const res = await request(app).post('/api/cover-letters').set('Cookie', [cookie]).send({ personaId: UUID })
    expect(res.status).toBe(400)
  })
