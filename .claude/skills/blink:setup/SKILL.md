...
3. **Always-loaded instructions.** Does `CLAUDE.md` exist, and does it contain a
...
5. **Skills.** For each Blink skill directory, check two things: presence and
   ...
   - `.claude/skills/blink:tracking/SKILL.md`
   - `.claude/skills/blink:setup/SKILL.md`
   - `.claude/skills/blink:sync/SKILL.md`

   Four new skills added in this version (report if missing):
   - `.claude/skills/blink:project/SKILL.md`
   - `.claude/skills/blink:design/SKILL.md`
   - `.claude/skills/blink:pause/SKILL.md`
   - `.claude/skills/blink:resume/SKILL.md`
...
6. **Legacy Blink artifacts.** Is `.claude/skills/blink-tracking/` there — the
...
7. **Agents.** Does `.claude/agents/` define anything? An empty roster is not an
...
which is the wrong answer for a file Blink itself wrote.
    ...
    | CLAUDE.md block            | missing            | append the Blink block                 |
    ...
    | .claude/skills/blink-tracking/ | legacy        | superseded by blink:tracking; remove |
...
about: nothing outside `.blink/`, `CLAUDE.md`, `AGENTS.md` and
`.claude/` is modified; every converted source file stays where it is; and the
...
Create `CLAUDE.md` and `AGENTS.md` if absent, append the marked block if the
...
Only `.claude/skills/blink-tracking/` qualifies, and only under all three of
these conditions.

1. **The replacement is really there.** `.claude/skills/blink:tracking/SKILL.md`
...
2. **Nobody edited the old one.** `blink init` wrote it and, as far as history
   shows, nothing has touched it since:
       ...
       git log --oneline -- .claude/skills/blink-tracking/SKILL.md
       git status --porcelain -- .claude/skills/blink-tracking/SKILL.md
...
`.claude/agents/` is empty and the roster was accepted, write these five.
...
Each is a `.claude/agents/<name>.md` with `name`, `description`, `tools` and a
...
If `.claude/agents/` already has definitions, leave them alone entirely. Report
...
- Do not delete a source file. The single exception is an unmodified legacy
  `.claude/skills/blink-tracking/`, under the three conditions above — and that
  is a file Blink wrote, not one anybody authored.
- Do not copy a percentage, a fraction or a count into any file.
- Do not overwrite agents, or anything outside the markers in `CLAUDE.md` and
  `AGENTS.md`.
