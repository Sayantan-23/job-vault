      ...
      c.projects = [
        {
          ...newProject(),
          name: 'JobVault',
          links: [{ ...newLink(), label: '', url: 'https://x.dev' }],
        },
      ];
      const errs = validateProfileContent(c);
      ...
      c.projects = [
        {
          ...newProject(),
          name: 'JobVault',
          links: [{ ...newLink(), label: 'Repo', url: 'https://x.dev' }],
        },
      ];
      expect(validateProfileContent(c)).toEqual([]);
    });
...
describe('formatMonthYear and formatMonthYearRange', () => {
  it('formats month and year', () => {
    expect(formatMonthYear({ month: 1, year: 2022 })).toBe('Jan 2022');
    expect(formatMonthYear({ month: null, year: 2020 })).toBe('2020');
  ...
  it('formats year-only sides', () => {
    expect(
      formatMonthYearRange({ month: null, year: 2019 }, { month: null, year: 2021 }, false)
    ).toBe('2019 – 2021');
  });

  it('formats full month+year on both sides', () => {
    expect(
      formatMonthYearRange({ month: 6, year: 2021 }, { month: 8, year: 2021 }, false)
    ).toBe('Jun 2021 – Aug 2021');
  });
