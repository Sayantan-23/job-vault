// Word-level diff for the "Fix grammar" review (where only a few words change).
...
export function diffWords(before: string, after: string): DiffSegment[] {
  ...
  // Longest common subsequence lengths (suffix DP). Reads default to 0 so we
  // never need non-null assertions on the grid.
