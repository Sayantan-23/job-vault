...
// Logic:
// 1. Define public routes: ['/login', '/register', '/auth/google/callback']
// 2. If route is public AND user is authenticated → redirect to /dashboard
// 3. If route is NOT public AND user is NOT authenticated → redirect to /login
...
- Register new user → redirected to dashboard → user menu shows name
- Login with credentials → redirected to dashboard
- Logout → redirected to login
- Access `/dashboard` while unauthenticated → redirected to login
