...
- [ ] **Next: Slice 7c — Cover Letters Workspace** (user-approved 2026-06-12 after a 7-agent market survey; **design spec ready**: `docs/superpowers/specs/2026-06-12-slice-7c-cover-letters-workspace-design.md`) — standalone `/app/cover-letters` workspace + paste-a-JD generation (`adhocJob` stored on the letter, `jobId` nullable, migration `0009`); JobDrawer flow unchanged. **Start by writing the TDD plan from the spec.** Then the backlog: **Chrome extension** (Slice 8), public-pages redesign, Google OAuth, email reminders (`docs/deferred-tasks.md`).
