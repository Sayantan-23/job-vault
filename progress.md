...
- [ ] **Manual browser verification outstanding** — load unpacked and confirm on a real Greenhouse/Workday form that the ATS's own character counter ticks up after Insert. If it stays at 0 the `input` event is not reaching their handler, and the field looks filled but submits empty. This is the one failure mode the unit tests cannot catch.
